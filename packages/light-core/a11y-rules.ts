/**
 * A11y Rules - Accessibility + Trauma-Informed Ruleset
 * Defines the baseline accessibility and trauma-informed issues
 */
...
/**
 * Base issues array - canonical list of accessibility and trauma-informed issues
 * AI must NOT freelance beyond this ruleset
 */
 ...
 * @returns A11yIssue or undefined
 */
export function getIssueById(id: string): A11yIssue | undefined {
  return BASE_ISSUES.find(issue => issue.id === id);
}

/**
 * Check if an issue is critical (must never pass)
 * @param issue - The issue to check
 * @returns true if issue is critical
 */
 ...
 */
export function getCriticalIssues(): A11yIssue[] {
  return BASE_ISSUES.filter(isCritical);
}

/**
 * Get all trauma-informed issues
 * @returns Array of trauma-informed issues
 */
export function getTraumaInformedIssues(): A11yIssue[] {
  return BASE_ISSUES.filter(issue => issue.traumaInformed === true);
}

/**
 * Get issues by WCAG level
 * @param level - 'A', 'AA', or 'AAA'
 * @returns Array of issues for that level
 */
export function getIssuesByWCAGLevel(level: 'A' | 'AA' | 'AAA'): A11yIssue[] {
  const levelPattern = new RegExp(`Level ${level}`, 'i');
  return BASE_ISSUES.filter(issue => levelPattern.test(issue.wcagRef));
}

/**
 * Validate that AI is not creating issues beyond the base ruleset
 * This is a constraint on AI freelancing
 * @param issueId - The issue ID to validate
 * @returns true if issue ID is in the base ruleset
 */
export function isValidIssueId(issueId: string): boolean {
  return BASE_ISSUES.some(issue => issue.id === issueId);
}
